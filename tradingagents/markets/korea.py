...
def instrument_context(ticker: str, curr_date: str | None) -> str:
    ...
    return build_instrument_context(ticker, "stock", identity, curr_date) + (
        ...
        " Scope missing-data statements to your own assigned report; do not claim other analysts "
        "received no data. Market analysis covers KIS prices; financial and news reports cover "
        "DART, Naver and ECOS separately. Current ECOS vintage is valid for today's analysis, "
        "not a reconstruction of historical releases. Distinguish observation date from publication date. "
        "For DART income, report current_3m separately from current_ytd and compare matching periods. "
        "Financial statements describe their reported accounting period, not just the past week's activity. "
        "Respect document_verified values and do not use unresolved conflicts in calculations. "
        "Do not issue request IDs or invent portfolio weights, share quantities, standard allocations "
        "or risk budgets. Without a supplied portfolio, any holding action must be conditional. "
        "A price target requires explicit valuation inputs and method; technical levels are not valuation targets."
    )
